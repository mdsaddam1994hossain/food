  ...
  const [targetProduct,setTergetProduct] = useState<TOrderHistory>()
    ...
    return (
          ...
          <div className='my-6'>
            {
               orderHistory?.map((item:TOrderHistory,index:number)=>{
                return(
                    ...
                    <motion.button onClick={()=>handleOpenModal(item)} whileTap={{scale:.9}} className=' mt-3 rounded-full w-full py-2 hover:border-none uppercase font-bold hover:text-white hover:bg-sky-700 duration-200 border border-black'>View Details</motion.button>
                 </div>
                )
               }) 
            }
        ...
        <div className="p-6 lg:p-8">
          <p className='text-xl font-bold text-gray-600'>{moment(targetProduct?.deliveryDate).format("DD MMM YYYY")}</p>    
          <p className='text-xl font-bold text-sky-950 mt-4'>BDT {targetProduct?.total}</p>
        </div>
        {
            ...
            return(
              <div key={index} className='p-6 lg:p-8'>
                ...
                <div className='flex justify-between'>
                  <p className='font-bold text-lg'>{item?.quantity} {item?.quantity > 1 ? "Pices" : "Pice"}</p>
                  <p className='font-bold text-lg text-sky-950'>BDT {item?.price}</p>
                </div>
